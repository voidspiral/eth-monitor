"""Wrap a user command with per-host ethernet collectors."""
...
def _start_remote_collector(
    ...
    *,
    ...
    result = ssh_run(host, start, user=user, identity=identity, timeout=timeout)
    ...
    *,
    ...
    for host in hosts:
        ...
        try:
            ...
            errors[host] = str(exc)
            print(f"eth-monitor: collect error on {host}: {exc}", file=sys.stderr)
    if remote_hosts:
    ...
    try:
        ...
        exit_code = 1
        print(f"eth-monitor: command failed to start: {exc}", file=sys.stderr)
    stop_file.touch()
